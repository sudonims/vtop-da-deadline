        ...
        for (let i = 0; i < table_inner.children.length; i++) {
            ...
            if (check == 0) {
                ...
                resolve({
                    ...
                    download: dwnld > 0 ? table_inner.children[i].children[5].children[0].children[0]
    ...
    for (let i = 1; i < table.children.length; i++) {
        ...
        if (table.children[i].children[3].children.length != 1) {
            await fetch("https://vtop.vit.ac.in/vtop/examinations/processDigitalAssignment", {
                ...
                var color = (due-now_)/(3600*24*1000) <= 2 ? "red" : "green";
                table.children[i].children[3].innerHTML += `<span style="display:inline; float:right; color:${color};">${due_date.due}</span>`;
