...
function find_right_due(table_inner) {
    return new Promise((resolve) => {
        ...
        for (let i = 0; i < table_inner.children.length; i++) {
            var check =
                table_inner.children[i].children[6].children[0].innerHTML;
            if (check === "") {
                var dwnld = table_inner.children[i].children[5].children.length;
                dueDates.push({
                    due: table_inner.children[i].children[4].children[0]
                        .innerHTML,
                    download:
                        dwnld > 0
            ...
            if (table.children[i].children[3].children.length != 1) {
                await fetch(
                    ...
                    .then(async (data) => {
                        ...
                        var table_inner =
                            doc.getElementsByClassName("customTable")[1]
                                .children[1];
                        ...
                        var due = new Date(
                            due_date.due.replace(/-/g, " ")
                        ).getTime();
                        var color =
                            (due - now_) / (3600 * 24 * 1000) <= 2
                                ? "red"
                                : "green";
                        table.children[
                            i
...
chrome_().runtime.onMessage.addListener(function (
    ...
    if (request.urlVisited && !loaded) {
        ...
        chrome_().storage.local.get(["token"], function (data) {
            var head = document.getElementsByClassName(
